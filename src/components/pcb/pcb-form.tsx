...
import { Plus, Trash2, AlertCircle, Link2, Sparkles } from "lucide-react"
...
export const emptyPcbLine = (): ManualLine => ({
  componentId: "",
  type: "",
  name: "",
  partNumber: "",
...
export interface PcbFormInitial {
  name?: string
  description?: string
  layers?: string
  status?: PcbStatus
...
type Props = {
  initial?: PcbFormInitial
  submitLabel?: string
  busy?: boolean
  onSubmit: (data: ManualPcbData) => void
  onCancel: () => void
}

/**
 * Shared PCB form (identity + BOM). Used by both "Add PCB" (modal) and
 * "Edit PCB" (page) so the two stay visually and behaviourally identical.
 */
export function PcbForm({ initial, submitLabel = "Save", busy = false, onSubmit, onCancel }: Props) {
  const { COMPONENTS } = useData()
  const [name, setName] = React.useState(initial?.name ?? "")
  const [description, setDescription] = React.useState(initial?.description ?? "")
  const [layers, setLayers] = React.useState(initial?.layers ?? "2")
  const [status, setStatus] = React.useState<PcbStatus>(initial?.status ?? "Prototype")
  const [lines, setLines] = React.useState<ManualLine[]>(
    initial?.lines?.length ? initial.lines : [emptyPcbLine(), emptyPcbLine()],
  )
  ...
  const addLine = () => setLines((prev) => [...prev, emptyPcbLine()])
    ...
    <div className="space-y-5">
      ...
      <div className="grid gap-4 sm:grid-cols-4">
        <div className="space-y-1.5 sm:col-span-2">
          <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">PCB Name *</label>
          <Input value={name} onChange={(e) => { setName(e.target.value); setError(null) }} placeholder="e.g. RoIP Main Board" />
        </div>
        <div className="space-y-1.5">
          <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Layers</label>
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between">
          <span className="text-[10px] uppercase font-bold text-muted-foreground/70 tracking-wider">Bill of Materials</span>
                            ...
                            {matches.map((c) => (
                              <li key={c.id}>
                                ...
                                >
                                  ...
                                  <span className="shrink-0 font-mono text-[10px] text-muted-foreground">{c.stock} in stock</span>
                                </button>
                              </li>
                            ))}
                          </ul>
              ...
              {lines.length === 0 && (
                <tr>
                  <td colSpan={7} className="px-3 py-6 text-center text-muted-foreground">
                    No lines. Click &ldquo;Add Line&rdquo;.
                  </td>
                </tr>
              )}
            </tbody>
